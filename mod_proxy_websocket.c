...
static apr_status_t ap_proxy_websocket_cleanup(const char *scheme,
                                          request_rec *r,
...
static
apr_status_t ap_proxy_websocket_cleanup(const char *scheme, request_rec *r,
                                   proxy_conn_rec *backend)
{
...
cleanup:
    if (backend) {
        if (status != OK)
            backend->close = 1;
        ap_proxy_websocket_cleanup(proxy_function, r, backend);
    }
    return status;
}
...
static void ap_proxy_websocket_register_hook(apr_pool_t *p)
{
...
module AP_MODULE_DECLARE_DATA proxy_websocket_module = {
    ...
    ap_proxy_websocket_register_hook/* register hooks */
};
